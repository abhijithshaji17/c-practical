...
int main() {
    ...
    for (i = 0; i < n; i++) {
        ...
        scanf("%d", &s[i].roll_no);
        printf("Name: ");
        scanf("%s", s[i].name); // Reads a single word name
        printf("Marks: ");
        scanf("%f", &s[i].marks);
    }

    // --- (a) Alphabetical List (Bubble Sort by Name) ---
    for (i = 0; i < n - 1; i++) {
        for (j = 0; j < n - i - 1; j++) {
    ...
    // --- (c) Sorted by Marks (Rank-wise - Descending) ---
    for (i = 0; i < n - 1; i++) {
        for (j = 0; j < n - i - 1; j++) {
            if (s[j].marks < s[j + 1].marks) { // '<' for highest marks first
                temp = s[j];
                s[j] = s[j + 1];
                s[j + 1] = temp;
            }
